...
void SceneGraphicsView::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (event->button() & Qt::LeftButton)
    {
        ...
        // If we were in rotating mode, stop rotating
        if (m_is_rotating) { m_is_rotating = false; }
    }
...
void SceneGraphicsView::rotateSelection(QPointF mouse_in_view)
{
    ...
    for (auto item : scene()->selectedItems()) {
        ...
        qreal angle = (angle_diff - m_last_angle_diff);
        QTransform t;
        ...
        //item->setRotation(my_rotation + angle);

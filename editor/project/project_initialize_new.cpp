...
void InitializeNewProject(DrProject *project, std::string project_name, Orientation orientation, int width, int height, bool test) {
    ...
    project->clearProject(true);                                        // #NOTE: Also adds Devices / Effects / Items
    ...
    if (test) {
        ...
        project->findWorldWithName("World 2")->getStageFromKey(start_stage)->addThing(DrThingType::Character, a_dragon->getKey(), 96,    96,   5);

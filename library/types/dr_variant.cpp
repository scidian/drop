...
double DrVariant::toDouble() {
    ...
    catch (const boost::bad_any_cast &) {   return 0.0; }
}
float DrVariant::toFloat() {
    try { return boost::any_cast<float>(m_value); }
    catch (const boost::bad_any_cast &) {   return 0.0f; }
}
int DrVariant::toInt() {
    try { return boost::any_cast<int>(m_value); }
    catch (const boost::bad_any_cast &) {   return 0; }
}
...
std::deque<DrVariant> DrVariant::toDeque() {
    try {
        return boost::any_cast<std::deque<DrVariant>>(m_value);
    }
    catch (const boost::bad_any_cast &) {
        try {
            std::vector<DrVariant> as_vector = boost::any_cast<std::vector<DrVariant>>(m_value);
            return { as_vector.begin(), as_vector.end() };
        }
        catch (const boost::bad_any_cast &) {
            return std::deque<DrVariant>();
        }
    }
}
...
std::vector<DrVariant> DrVariant::toVector() {
    try {
        ...
        return boost::any_cast<std::vector<DrVariant>>(m_value);
    }
    catch (const boost::bad_any_cast &) {
        try {
            std::deque<DrVariant> as_deque = boost::any_cast<std::deque<DrVariant>>(m_value);
